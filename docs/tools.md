...
If an older target still contains `skills/zsh-agent-workflow/`, the installer reports it. Pass `-RemoveLegacySkill` with `-Apply` to remove the stale skill directory during an upgrade.
