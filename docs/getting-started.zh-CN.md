...
7. 在你要开发的项目仓库里启动 Codex。如果宿主没有自动加载已安装的指令，可以把下面这段作为第一条提示词：
...
4. 只有在理解生成文件后，再把渲染后的 hook settings 合并进 Claude Code settings。hook 生命周期和安全边界见 [hook-runtime.zh-CN.md](hook-runtime.zh-CN.md)。
...
2. [workflow-examples.zh-CN.md](workflow-examples.zh-CN.md)：bug 修复、feature、review、长任务和发布检查的提示词示例。
3. [tools.zh-CN.md](tools.zh-CN.md)：具体命令和工具行为。
4. [hook-runtime.zh-CN.md](hook-runtime.zh-CN.md)：生命周期 hook 示例，以及它们能和不能强制什么。
