...
- 面向 public tools、hook runtime 和 release surface 的聚焦验证脚本
...
- TDD 和 independent review 的质量要求
...
- `tools/validate-release-readiness.ps1` 验证公开 release assets、Markdown 链接、fresh workspace 安装、渲染后的 host configs 和安装后的 hook smoke tests。
- `tools/validate-phase3.ps1` 验证 public tool surface 和 installer 行为。
- `tools/validate-runtime-slice.ps1` 验证 hook runtime 纵切。
- `tools/test-agent-hooks.ps1` 通过 stdin 发送真实 hook event JSON，做端到端 smoke coverage。

Maintainer 发布检查见 [docs/release-checklist.zh-CN.md](docs/release-checklist.zh-CN.md)，项目叙事见 [docs/resume-case-study.zh-CN.md](docs/resume-case-study.zh-CN.md)。
