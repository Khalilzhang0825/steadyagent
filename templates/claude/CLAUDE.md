# CLAUDE.md - SteadyAgent Claude Code Template

Use this file as a copy-ready Claude Code instruction template. Copy this template together with the `rules/` directory, or adjust the rule paths after copying. Keep it short. Load detailed rules from `rules/` only when the task needs them.
...
- `rules/review-gates.md` for independent review, subagents, and scoring.
...
- Claude Code can use hooks when the user configures them.
- Hooks are useful for stable safety checks, not for early exploration.
- Use subagents for independent review, broad read-only research, or parallel work on disjoint files.
